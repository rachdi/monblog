...
(function() {
  ...
  generate = function(context, text) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (args = line.match(/^([0-9.eE-]+) +([\w \/%(){},&-]+)$/)) {
        ...
        results.push(emitrow(context, args[1], "=B" + (context.vars[args[1]] + 1)));
      } else {
        results.push(emitrow(context, "can't parse '" + line + "'"));
      }
    }
    return results;
  };
  ...
  compile = function(program, titles, done) {
    ...
    fetch = (function() {
      ...
      for (j = 0, len = titles.length; j < len; j++) {
        title = titles[j];
        results.push($.getJSON("/" + (wiki.asSlug(title.title)) + ".json"));
      }
      return results;
    })();
    return $.when.apply($, fetch).then(function() {
      var context, item, j, k, len, len1, ref, xhr, xhrs;
      xhrs = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      context = {
        ops: [],
        vars: {}
      };
  ...
  prefetch = function(titles, done) {
    ...
    return $.when.apply($, fetch).then(function() {
      var i, item, j, k, len, ref, ref1, xhr, xhrs;
      xhrs = 1 <= arguments.length ? slice.call(arguments, 0) : [];
